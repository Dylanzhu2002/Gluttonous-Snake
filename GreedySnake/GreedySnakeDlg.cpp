...
void CGreedySnakeDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	{
		KillTimer(1);
		if (isSingle) MessageBox(TEXT("Game Over!"), TEXT("Warning"));
		else MessageBox(TEXT("Game Over!"), TEXT("Warning"));
		return;
	}
	...
	if (!isSingle)
	{
		...
		{
			KillTimer(1);
			MessageBox(TEXT("Game Over! Player 1 wins!"), TEXT("Warning"));
			return;
		}
	}
	...
	if (snake1.ateFood)	//如果吃到了食物
	{
		...
		{
			KillTimer(1);
			MessageBox(TEXT("Game Over! Player 1 reach 20 point, Player 1 wins!"), TEXT("Warning"));
			return;
		}
	...
	if (!isSingle && snake2.ateFood)
	{
		...
		{
			KillTimer(1);
			MessageBox(TEXT("Game Over! Player 2 reach 20 point,Player 2 wins!"), TEXT("Warning"));
			return;
		}
